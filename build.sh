...
j7xelte)
	case $VARIANT in
	can|duos|eur|xx)
		KERNEL_DEFCONFIG=exynos7870-j7xelte_eur_open_defconfig
		;;
	*)
		echo "Unknown variant: $VARIANT"
		exit 1
	...
	j7xelte)
		...
		can|duos|eur|xx)
			DTSFILES="exynos7870-j7xelte_eur_open_00 exynos7870-j7xelte_eur_open_01
					exynos7870-j7xelte_eur_open_02 exynos7870-j7xelte_eur_open_03 exynos7870-j7xelte_eur_open_04"
			;;
		*)
			echo "Unknown variant: $VARIANT"
			exit 1
...
FUNC_BUILD_RAMDISK()
{
	...
	j7xelte)
		case $VARIANT in
		can|duos|eur|xx)
			rm -f $RDIR/ramdisk/SM-J710F/split_img/boot.img-zImage
			rm -f $RDIR/ramdisk/SM-J710F/split_img/boot.img-dtb
			mv -f $RDIR/arch/$ARCH/boot/boot.img-zImage $RDIR/ramdisk/SM-J710F/split_img/boot.img-zImage
			mv -f $RDIR/arch/$ARCH/boot/boot.img-dtb $RDIR/ramdisk/SM-J710F/split_img/boot.img-dtb
			cd $RDIR/ramdisk/SM-J710F
			./repackimg.sh
			echo SEANDROIDENFORCE >> image-new.img
			;;
		*)
...
FUNC_BUILD_ZIP()
{
	...
	j7xelte)
		case $VARIANT in
		can|duos|eur|xx)
			mv -f $RDIR/ramdisk/SM-J710F/image-new.img $RDIR/build/$MODEL-$VARIANT.img
			;;
		*)
			echo "Unknown variant: $VARIANT"
			exit 1
